...
@app.get("/api/calibration/status", tags=["Core"], summary="Calibration baseline info")
async def calibration_status():
    """Check whether a calibration baseline is loaded."""
    if not engine.calibration or not engine.calibration.is_ready:
        return {"calibrated": False}
    cal = engine.calibration
    return {
        "calibrated": True,
        "source":     cal.source_hint,
        "n_images":   cal.n_images,
        "k_sigma":    cal.K_SIGMA,
    }


@app.post("/admin/calibration/clear", tags=["Admin"], summary="Reset calibration baseline")
async def calibration_clear():
    """Remove the calibration baseline and revert to hardcoded thresholds."""
